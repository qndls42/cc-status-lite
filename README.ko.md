...
설치 스크립트가 확인하는 것도, 보통 따로 깔아야 하는 것도 `jq` 하나뿐입니다.

> [!NOTE]
> 상태줄은 `curl`과 `git`도 씁니다. 둘 다 macOS와 주요 리눅스에 기본으로 들어 있어서 보통은 할 일이 없지만, 슬림 컨테이너 이미지나 최소 설치 서버처럼 없는 환경이라면 같이 깔아주세요.
>
> ```bash
> sudo apt install curl git
> ```
>
> `curl`은 API에서 5시간·주간 한도 값을 받아옵니다. 없으면 그 두 칸이 아예 안 나옵니다. `git`은 브랜치 이름을 읽습니다. 없으면 브랜치만 빠집니다. 나머지는 어느 경우든 그대로 나옵니다.
